...
function scannedQrKey(){ return scopedKey(SCANNED_QR_KEY); }
...
document.addEventListener('DOMContentLoaded', () => {
  if (document.body.dataset.page === 'login') {
    ...
    // Initialize gold leaf pattern if not exists
    if (!localStorage.getItem('barns-gold-leaf-pattern')) {
      generateRandomGoldPattern();
    }
  }
});
...
// Enhanced Invoice Storage Functions with File Persistence
async function loadScannedQrCodes() {
  try {
    // Try to use the new invoice storage system first
    ...
    const raw = localStorage.getItem(scannedQrKey());
...
async function saveScannedQrCode(qrData, timestamp) {
  try {
    // Always try to use the new global invoice storage system first
    if (window.invoiceStorage) {
      const success = await window.invoiceStorage.saveInvoice(qrData, qrData);
      if (success) {
        console.log(`QR code ${qrData} saved globally - preventing duplicate scans by other users`);
        return true;
      } else {
        console.warn(`QR code ${qrData} already exists globally - duplicate scan prevented`);
        return false;
      }
    }
    
    // Fallback to localStorage (should not happen in normal operation)
    console.warn('Global invoice storage not available, falling back to localStorage');
    const codes = await loadScannedQrCodes();
    codes[qrData] = {
      timestamp: timestamp,
      user: userName || 'Anonymous'
    };
    localStorage.setItem(scannedQrKey(), JSON.stringify(codes));
    return true;
  } catch (error) {
...
async function isQrCodeAlreadyScanned(qrData) {
  try {
    // Always try to use the new global invoice storage system first
    if (window.invoiceStorage) {
      const exists = await window.invoiceStorage.isInvoiceExists(qrData);
      if (exists) {
        console.log(`QR code ${qrData} already exists globally - preventing duplicate scan`);
      }
      return exists;
    }
    
    // Fallback to localStorage (should not happen in normal operation)
    console.warn('Global invoice storage not available, falling back to localStorage check');
    const codes = await loadScannedQrCodes();
    return codes.hasOwnProperty(qrData);
  } catch (error) {
    console.error('Error checking if QR code already scanned:', error);
    return false;
  }
}

async function getQrCodeScanInfo(qrData) {
  try {
    // Always try to use the new global invoice storage system first
    if (window.invoiceStorage) {
      const invoice = await window.invoiceStorage.getInvoiceInfo(qrData);
      if (invoice) {
        return {
          timestamp: invoice.timestamp,
          user: invoice.user,
    ...
    console.warn('Global invoice storage not available, falling back to localStorage');
    const codes = await loadScannedQrCodes();
    return codes[qrData] || null;
  } catch (error) {
    console.error('Error getting QR code scan info:', error);
    return null;
  }
}
...
function addOneLeafToMap(isGold) {
  ...
  // Get current green leaves count to determine if we should add a gold leaf
  const currentGreenCount = map.filter(leaf => leaf === 'green').length;
  const shouldBeGold = shouldAddGoldLeaf(currentGreenCount);
...
// Function to determine if we should add a gold leaf based on current green count
function shouldAddGoldLeaf(currentGreenCount) {
  // Get the gold leaf pattern from localStorage or create a new one
  let goldPattern = localStorage.getItem('barns-gold-leaf-pattern');
  
  if (!goldPattern) {
    // Create a new random pattern for every 5 green leaves
    goldPattern = generateRandomGoldPattern();
    localStorage.setItem('barns-gold-leaf-pattern', goldPattern);
  }
  
  // Parse the pattern
  const pattern = JSON.parse(goldPattern);
  
  // Check if current green count matches any position in the pattern
  return pattern.includes(currentGreenCount + 1); // +1 because we're about to add a new leaf
}

// Function to generate a random pattern for gold leaves
function generateRandomGoldPattern() {
  // Generate a random position from 1 to 5 for the gold leaf
  const goldPosition = Math.floor(Math.random() * 5) + 1;
  
  // Create pattern array with the gold position
  const pattern = [];
  for (let i = 1; i <= 5; i++) {
    if (i === goldPosition) {
      pattern.push(i);
  ...
  console.log(`Generated new gold leaf pattern: Gold leaf will appear at position ${goldPosition} of the next 5 leaves`);
  return JSON.stringify(pattern);
}

// Function to reset gold pattern (can be called manually if needed)
function resetGoldLeafPattern() {
  localStorage.removeItem('barns-gold-leaf-pattern');
  console.log('Gold leaf pattern manually reset');
}
...
async function handleQrResult(data) {
  ...
  try {
    // Check if QR code has already been scanned globally (by ANY user)
    const alreadyScanned = await isQrCodeAlreadyScanned(data);
    if (alreadyScanned) {
      // Get scan information for duplicate
      const scanInfo = await getQrCodeScanInfo(data);
      const timestamp = scanInfo ? scanInfo.timestamp : Date.now();
      const scannedBy = scanInfo ? scanInfo.user : 'Unknown User';
      
      // Redirect directly to tree page for duplicate scans
      try {
        alert(`This invoice has already been scanned by ${scannedBy}!`);
    ...
    // Save the QR code as scanned globally (prevents other users from scanning)
    const timestamp = Date.now();
    const saveSuccess = await saveScannedQrCode(data, timestamp);
    
    if (!saveSuccess) {
      console.warn('Failed to save invoice data globally, but continuing with scan');
      // Show warning but continue with scan
      alert('Warning: Could not save invoice globally. Other users might be able to scan this code.');
    } else {
    ...
    // IMPORTANT: Don't redirect to URLs - just process the scan
    // Remove the URL redirection logic completely
    
    // Redirect directly to rate page for successful scans
    try {
      window.location.href = 'rate.html';
    } catch {
      // Fallback to tree page
      window.location.href = 'tree.html';
    }
    
    // Don't add green leaf here - only coffee counter
    // state.leaves = Math.min(LEAVES_COUNT, state.leaves + 1);
    // const res = addOneLeafToMap(false);
    ...
    // Successfully redirected to rate.html
  } catch (error) {
...
function mountPageSpecific() {
  ...
  // Chat functionality is now handled by the dedicated AI integration module
  // See assets/js/ai.js for the complete implementation
  if (page === 'chat') {
    // The chat page now uses the initAIForm function from ai.js
    // This provides better separation of concerns and easier maintenance
    console.log('Chat page loaded - AI integration handled by ai.js module');
    
    // تعطيل الكود القديم ولكن لا نوقف التنفيذ
    // return; // تم إزالة هذا السطر
  ...
  if (page === 'tree') {
    ...
    // الصور ثابتة دائماً - لا تتغير بغض النظر عن المستخدم الحالي
